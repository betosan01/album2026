...
def cargar_datos_desde_google():
    try:
        ...
        st.error(f"🚨 ¡ALTO AHÍ! Falla en la Escena del Crimen: {str(e)}")
        return False
...
def registrar_log_remoto(accion):
    ...
    try:
        ...
        st.sidebar.warning(f"⚠️ No se pudo actualizar la bitácora: {str(e)}")
...
def calcular_insignias(df_rank, df_completo, df_logs):
    ...
    if any(v > 0 for v in tratos_count.values()):
    ...
    if st.button("🔄 Actualizar Cantidades 🛠️"):
        ...
        with st.spinner("Ajustando inventario..."):
            ...
            try:
                ...
                st.error(f"🚨 Error real al ajustar inventario: {str(e)}")
                ...
                if st.session_state.ultima_transaccion == transaccion_baja:
                    ...
                    with st.spinner("Ejecutando bajas..."):
                        ...
                        try:
                            ...
                            st.error(f"🚨 Error real al procesar bajas: {str(e)}")
...
with t2:
    # TRATOS DIRECTOS OPTIMIZADOS (TODOS LOS CANJES POSIBLES)
    for o in nombres_papus:
        if o != usuario:
            ...
            if yo_tengo_el_no and el_tiene_yo_no:
                st.markdown(f"#### 🤝 Tratos con {o}")
                # Generar todas las combinaciones 1:1
                # Para no saturar, mostramos en columnas compactas
                cols_t = st.columns(3)
                for idx_t, (mia, suya) in enumerate(zip(yo_tengo_el_no, el_tiene_yo_no)):
                    with cols_t[idx_t % 3]:
                        with st.container(border=True):
                            st.markdown(f"**Doy:** `{mia}`  \n**Recibo:** `{suya}`")
                            if st.button(f"Cambiar 🔁", key=f"t1x1_{o}_{mia}_{suya}"):
                                with st.spinner("Firmando el contrato..."):
                                    df.at[df[df['ESTAMPA'] == mia].index[0], usuario] -= 1
                                    df.at[df[df['ESTAMPA'] == mia].index[0], o] += 1
                                    df.at[df[df['ESTAMPA'] == suya].index[0], o] -= 1
                                    df.at[df[df['ESTAMPA'] == suya].index[0], usuario] += 1
                                    try:
                                        conn.update(spreadsheet=url_del_sheet, worksheet="SHEET1", data=df)
                                        registrar_log_remoto(f"🤝 TRATO: {usuario} y {o} cambiaron {mia} x {suya}")
                                        st.session_state.df_maestro = df
                                        st.success("🤝 ¡Trato exitoso!")
                                        time.sleep(1); st.rerun()
                                    except Exception as e: st.error(f"🚨 Error: {e}")

with t3:
    # TRÍOS OPTIMIZADOS (TODAS LAS CARAMBOLAS POSIBLES)
    ...
    for b in otros:
        for c in [p for p in otros if p != b]:
            ...
            # Buscamos todas las combinaciones que cierren el ciclo
            for e1, e2, e3 in zip(a_a_b, b_a_c, c_a_a):
                carambolas.append({"b": b, "c": c, "e1": e1, "e2": e2, "e3": e3})

    if carambolas:
        st.write("### 🔄 Carambolas de 3 Bandas")
        cols_3 = st.columns(2)
        for idx, tr in enumerate(carambolas):
            with cols_3[idx % 2]:
                with st.container(border=True):
                    st.markdown(f"**Ciclo:** Tú ➔ `{tr['e1']}` ➔ **{tr['b']}** ➔ `{tr['e2']}` ➔ **{tr['c']}** ➔ `{tr['e3']}` ➔ Tú")
                    if st.button(f"Armar la machaca 🔄", key=f"btn_3_{idx}"):
                        with st.spinner("Haciendo el cambalache..."):
                            df.at[df[df['ESTAMPA'] == tr['e1']].index[0], usuario] -= 1
                            df.at[df[df['ESTAMPA'] == tr['e1']].index[0], tr['b']] += 1
                            df.at[df[df['ESTAMPA'] == tr['e2']].index[0], tr['b']] -= 1
                            df.at[df[df['ESTAMPA'] == tr['e2']].index[0], tr['c']] += 1
                            df.at[df[df['ESTAMPA'] == tr['e3']].index[0], tr['c']] -= 1
                            df.at[df[df['ESTAMPA'] == tr['e3']].index[0], usuario] += 1
                            try:
                                conn.update(spreadsheet=url_del_sheet, worksheet="SHEET1", data=df)
                                registrar_log_remoto(f"🔄 TRÍO: {usuario}➔{tr['b']}➔{tr['c']}")
                                st.session_state.df_maestro = df
                                st.success("🔄 ¡Ciclo completado!")
                                time.sleep(1); st.rerun()
                            except Exception as e: st.error(f"🚨 Error: {e}")
    else:
        st.write("No hay carambolas por ahora. 😶")
