...
# Auto-creación de columnas de prioridad si no existen
...
for p in nombres_papus:
    ...
    rank_data.append({
        ...
        "PUNTOS": (pegadas * 2) + int(repetidas) # Ranking por actividad y colección
    })
...
with st.sidebar:
    ...
    # Ajuste matemático para sobres de 7 estampas
    prob_nueva = 1 - ( ( (total_total - faltantes) / total_total ) ** 7 )
    sobres_estimados_totales = (total_total * np.log(total_total) + 0.577 * total_total) / 7
...
# --- 3. CAZA-TRATOS (MERCADO NIGGER OPTIMIZADO) ---
st.divider()
...
with tab_caza:
    ...
    if not tratos_encontrados: st.info("No hay tratos de 'toma y daca' listos todavía.")
...
chunk = df_display.iloc[st.session_state.album_page*ITEMS_PAG : (st.session_state.album_page+1)*ITEMS_PAG]
...
with col_g1:
    no_prio = df[(df[col_prioridad_mia] == 0) & (df[usuario] == 0)]['ESTAMPA'].tolist()
    if no_prio:
        p_add = st.selectbox("Marcar como Dorada:", no_prio, key="add_g")
...
with col_g2:
    si_prio = df[df[col_prioridad_mia] > 0]['ESTAMPA'].tolist()
    if si_prio:
        p_rem = st.selectbox("Quitar de Doradas:", si_prio, key="rem_g")
